...
import { WhatsAppAgentSetup } from './components/agent-whatsapp/WhatsAppAgentSetup';
import { api, apiFetch } from './lib/api';
import { Image as ImageIcon, Video, ArrowRight, Activity, Zap, Mic, X, ChevronLeft, ChevronRight, Bell, Check, Megaphone, Loader2, CheckCircle2, AlertCircle, Globe, Bot, LogOut, Music, MessageCircle, Target } from 'lucide-react';
...
import UserSupportChat from './components/ui/UserSupportChat';
...
export default function App() {
  ...
  const { unreadCount, setUnreadCount } = useAgentRealtime(user?.id);
  const [toast, setToast] = useState<{title: string, message: string, type: 'message' | 'order'} | null>(null);
  ...
  React.useEffect(() => {
    ...
    const handleAgentNotification = (e: any) => {
        setToast(e.detail);
    ...
    window.addEventListener('agent_notification', handleAgentNotification);
    return () => {
        window.removeEventListener('session-expired', handleSessionExpired);
        window.removeEventListener('agent_notification', handleAgentNotification);
    };
  }, []);
  ...
  const handleNavigate = (page: string) => {
    ...
    // Clear unread if entering agent pages
    if (page.startsWith('agent-whatsapp')) {
        setUnreadCount(0);
    }
  ...
  const renderContent = () => {
      ...
      return (
        <>
          ...
          <section className="mt-12 mb-8">
            <div className="relative overflow-hidden rounded-[2.5rem] p-[1px] bg-gradient-to-r from-[#FFB800]/50 via-accent/30 to-[#FFB800]/50 shadow-2xl">
              <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-10 mix-blend-overlay"></div>
              <div className="relative bg-[#0A0A0A]/95 backdrop-blur-2xl rounded-[calc(2.5rem-1px)] p-8 md:p-10 flex flex-col md:flex-row items-center justify-between gap-8 border border-white/5 overflow-hidden">
                <div className="absolute top-0 right-0 w-96 h-96 bg-[#FFB800]/10 rounded-full blur-[100px] -translate-y-1/2 translate-x-1/3"></div>
                <div className="absolute bottom-0 left-0 w-64 h-64 bg-accent/10 rounded-full blur-[80px] translate-y-1/2 -translate-x-1/3"></div>
                
                <div className="flex-1 relative z-10 text-center md:text-left">
                  <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#FFB800]/20 border border-[#FFB800]/30 text-[#FFB800] text-[10px] font-black uppercase tracking-widest mb-4">
                  ...
                  <h3 className="text-3xl md:text-4xl font-black text-white mb-3 tracking-tight">Agentes de Vendas no <span className="text-[#25D366]">WhatsApp</span></h3>
                  <p className="text-text-secondary text-base md:text-lg max-w-2xl leading-relaxed">
                    A tua equipa de vendas vai ser automatizada. Um <strong>CRM integrado</strong>, <strong>Funil de Vendas Inteligente</strong> e agentes autónomos que respondem, qualificam e fecham clientes 24/7.
                  </p>
                </div>
                
                <div className="relative z-10 shrink-0">
                  <div className="w-24 h-24 md:w-32 md:h-32 rounded-3xl bg-gradient-to-br from-[#25D366]/20 to-[#FFB800]/20 border border-white/10 flex items-center justify-center shadow-[0_0_50px_rgba(37,211,102,0.15)] group relative overflow-hidden">
                    <div className="absolute inset-0 bg-white/5 group-hover:bg-white/10 transition-colors"></div>
                    <Bot className="w-12 h-12 md:w-16 md:h-16 text-[#FFB800] animate-pulse" strokeWidth={1.5} />
                    <div className="absolute -bottom-2 -right-2 w-12 h-12 bg-[#25D366] rounded-full flex items-center justify-center shadow-lg border-4 border-[#0A0A0A]">
                      <Target className="w-5 h-5 text-[#0A0A0A]" strokeWidth={2.5} />
                    </div>
                  </div>
                </div>
              </div>
    ...
    if (currentPage === 'agent-whatsapp-setup') {
      return <WhatsAppAgentSetup />;
    }

    if (currentPage === 'agent-whatsapp-crm') {
      return <CRMPanel />;
    }

    if (currentPage === 'marketing') {
      // Redirect to the static HTML file
      window.location.href = '/conversio-agents-marketing.html';
      return null;
    }
    ...
    <div className="min-h-screen w-full relative font-sans selection:bg-white/20">
      ...
      {toast && (
        <div className={`fixed top-6 right-6 z-[200] animate-in slide-in-from-right fade-in duration-500`}>
          <div className={`p-5 rounded-[2rem] border backdrop-blur-xl shadow-2xl flex items-center gap-4 max-w-sm ${
            toast.type === 'order' 
            ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-500' 
            : 'bg-[#FFB800]/10 border-[#FFB800]/20 text-[#FFB800]'
          }`}>
            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 ${
              toast.type === 'order' ? 'bg-emerald-500/20' : 'bg-[#FFB800]/20'
            }`}>
              {toast.type === 'order' ? <ShoppingCart size={24} /> : <MessageCircle size={24} />}
            </div>
            <div className="flex-1 min-w-0">
              <h4 className="text-sm font-black uppercase tracking-tight truncate">{toast.title}</h4>
              <p className="text-xs font-medium opacity-70 truncate mt-0.5">{toast.message}</p>
            </div>
            <button onClick={() => setToast(null)} className="p-2 hover:bg-white/10 rounded-full transition-all">
              <X size={16} />
            </button>
          </div>
        </div>
      )}
      {/* Floating Support Chat for Users */}
      {!['landing', 'auth', 'splash', 'video-cores-info', 'image-cores-info'].includes(currentPage) && (
        <UserSupportChat />
      )}
    </div>
  );
