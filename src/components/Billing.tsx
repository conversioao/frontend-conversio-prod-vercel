               ...
               <div className="relative z-10 flex flex-col h-full">
                  <div className="flex-1">
                     ...
                     <div className="border-t border-border-subtle pt-6 space-y-4">
                        ...
                        <div className="flex justify-between text-sm text-text-secondary">
                           <span className="flex items-center gap-2"><Mic size={14} className="text-emerald-500" /> Músicas AI</span>
                      ...
                      <button onClick={() => {
                        ...
                        <div className="flex items-center gap-5">
                          ...
                          <div className="text-left">
                            ...
                            <span className="block text-xs text-text-tertiary mt-1">Transferência instantânea (Nº {paymentInfo?.mcx_express?.[0]?.number || '944442149'})</span>
                          </div>
                        </div>
                     ...
                     <div className="space-y-6 flex-1">
                        {paymentMethod === 'Pagamento por Referência' && (
                           <div className="bg-[#FFB800]/5 border border-[#FFB800]/20 p-6 rounded-2xl space-y-3">
                             ...
                             <div className="space-y-3">
                                ...
                                <div onClick={() => navigator.clipboard.writeText(paymentInfo?.mcx_express?.[0]?.number?.replace(/\s/g, '') || '944442149')} className="flex justify-between items-center p-4 bg-black/40 rounded-xl border border-white/5 cursor-pointer hover:border-[#FFB800]/30 transition-all">
                                   <span className="text-[10px] font-black text-text-tertiary uppercase tracking-widest">Referência</span>
                                   <div className="flex items-center gap-2">
                                      <span className="text-xl font-black text-[#FFB800] tracking-widest">{paymentInfo?.mcx_express?.[0]?.number || '944 442 149'}</span>
                                      <ReceiptText size={16} className="text-[#FFB800]" />
                                   </div>
                                </div>
                             </div>
                        ...
                        {paymentMethod === 'PayPay e Kwik' && (
                           <div className="bg-emerald-500/5 border border-emerald-500/20 p-6 rounded-2xl space-y-3">
                             ...
                             <div className="space-y-3">
                                <div onClick={() => navigator.clipboard.writeText(paymentInfo?.mcx_express?.[0]?.number?.replace(/\s/g, '') || '944442149')} className="flex justify-between items-center p-4 bg-black/40 rounded-xl border border-white/5 cursor-pointer hover:border-emerald-500/30 transition-all">
                                   <span className="text-[10px] font-black text-text-tertiary uppercase tracking-widest">Número de Pagamento</span>
                                   <div className="flex items-center gap-2">
                                      <span className="text-xl font-black text-white tracking-widest">{paymentInfo?.mcx_express?.[0]?.number || '944 442 149'}</span>
                             ...
                             <div className="pt-2 border-t border-emerald-500/10">
                                <p className="text-[10px] text-emerald-500 font-bold uppercase">Beneficiário: <span className="text-white">{paymentInfo?.beneficiary_name || 'CONVERSIO AO'}</span></p>
                             </div>
                           </div>
                        )}
