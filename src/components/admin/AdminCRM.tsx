...
const AdminCRM: React.FC<AdminCRMProps> = ({ onClose }) => {
    ...
    const [drawerTab, setDrawerTab] = useState<'geral' | 'whatsapp' | 'compras' | 'geracoes'>('geral');
    ...
    const fetchUserDetails = async (lead: Lead) => {
        ...
        if (data.success) {
            ...
            setWhatsappLogs(data.whatsapp_logs || []);
            setSelectedLeadData(data.user);
        }
    };
    ...
    return (
        <>
        <div className="fixed inset-0 z-[80] bg-zinc-950 text-white flex flex-col font-sans overflow-hidden">
            
            {/* Minimalist Professional Header */}
            <div className="h-16 border-b border-zinc-800 flex items-center justify-between px-8 bg-zinc-950 shrink-0">
                <div className="flex items-center gap-6">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-sm font-bold text-white tracking-tight">Sales CRM Intelligence</h1>
                            <p className="text-[10px] text-zinc-500 font-medium uppercase tracking-wider">Overview & Pipeline Control</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-4">
                    ...
                    <button 
                         ...
                         className="px-4 py-2 bg-amber-500 text-black rounded-lg text-[10px] font-bold uppercase tracking-wider hover:opacity-90 transition-all flex items-center gap-2"
                    >
            ...
            <AnimatePresence mode="wait">
                {activeTab === 'pipeline' && (
                    <motion.div 
                        ...
                        className="flex gap-6 overflow-x-auto pb-8 items-start no-scrollbar"
                    >
                        {stages.map(stage => (
                            <div 
                                key={stage.id} 
                            ...
                            >
                                <div className="flex items-center justify-between px-4 py-3 bg-zinc-900/50 border border-zinc-800 rounded-xl">
                                    <h3 className="text-[10px] font-bold text-zinc-300 uppercase tracking-wider flex items-center gap-2">
                                ...
                                <div className={`flex flex-col gap-3 min-h-[500px] rounded-xl transition-colors ${draggedLeadId ? 'bg-amber-500/5 border border-dashed border-amber-500/20' : 'bg-transparent'}`}>
                                    {leads.filter(l => String(l.crm_stage_id) === String(stage.id)).length === 0 ? (
                                        <div className="flex-1 flex items-center justify-center text-[9px] font-bold text-zinc-700 uppercase tracking-widest italic text-center px-8">
                                            Vazio
                                        </div>                                    ) : (
                                        leads.filter(l => String(l.crm_stage_id) === String(stage.id)).map(lead => (
                                            <motion.div
                                                ...
                                                layoutId={lead.id}
                                                className="bg-zinc-900 border border-zinc-800 p-5 rounded-xl hover:border-amber-500/30 cursor-grab active:cursor-grabbing transition-all group shadow-sm"
                                            >
                                                <div className="flex items-center justify-between mb-3">
                                                    <div className="font-bold text-sm text-white group-hover:text-amber-500 transition-colors tracking-tight flex items-center gap-2">
                                                        {lead.name}
                                                        {lead.temperature === 'hot' && <Zap size={10} className="text-amber-500 fill-amber-500" />}
                                                    </div>
                                                    <div className="flex items-center gap-2">
                                                        <div className={`px-2 py-0.5 rounded text-[8px] font-bold uppercase tracking-widest ${
                                                            lead.crm_stage_id === '9' || lead.category === 'VIP' ? 'bg-amber-500 text-black' : 
                                                            lead.category === 'Qualified' ? 'bg-emerald-500 text-white' : 
                                                            'bg-zinc-800 text-zinc-500 border border-zinc-700'
                                                        }`}>
                                                            {lead.crm_stage_id === '9' ? 'USUÁRIO' : (lead.category || 'Lead')}
                                                        </div>
                                                        <div className={`w-1.5 h-1.5 rounded-full ${lead.last_login_at && (Date.now() - new Date(lead.last_login_at).getTime() < 5 * 60 * 1000) ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)] animate-pulse' : 'bg-zinc-700'}`} title={lead.last_login_at && (Date.now() - new Date(lead.last_login_at).getTime() < 5 * 60 * 1000) ? 'Online' : 'Offline'} />
                                                    </div>
                                                </div>

                                                <div className="flex items-center justify-between mb-4">
                                                     <div className="text-[10px] font-medium text-zinc-500 flex items-center gap-2">
                                                         <Phone className="w-3 h-3 text-amber-500/50" /> {lead.whatsapp}
                                                     </div>
                                                </div>
                                                
                                                <div className="flex items-center justify-between pt-3 border-t border-zinc-800/50">
                                                    <div className="flex items-center gap-1.5 text-[9px] font-bold text-zinc-600 uppercase tracking-wider" title="Data de Entrada">
            ...
            {/* Lead Details Drawer */}
            {selectedLead && (
                <div className="fixed inset-0 z-[90] flex items-center justify-end p-4 bg-black/60 backdrop-blur-sm" onClick={() => setSelectedLead(null)}>
                    <motion.div 
                        initial={{ x: 500 }}
                        animate={{ x: 0 }}
                        onClick={(e) => e.stopPropagation()}
                        className="w-full max-w-md h-full bg-zinc-950 border-l border-zinc-800 p-8 shadow-2xl relative overflow-y-auto no-scrollbar rounded-l-2xl"
                    >
                        <button onClick={() => setSelectedLead(null)} className="absolute top-6 left-6 p-2 text-zinc-500 hover:text-white transition-colors">
                            <X className="w-5 h-5" />
                        </button>

                        <div className="mt-12 space-y-10">
                            <div className="text-center">
                                <div className="w-16 h-16 rounded-2xl bg-amber-500/10 flex items-center justify-center text-amber-500 font-bold text-2xl border border-amber-500/20 mx-auto mb-4">
                                    {selectedLead.name.charAt(0)}
                                </div>
                                <h2 className="text-xl font-bold text-white tracking-tight mb-2">{selectedLead.name}</h2>
                                <div className="flex items-center justify-center gap-2">
                                     <span className="text-[9px] font-bold text-amber-500 uppercase tracking-widest px-2 py-0.5 bg-amber-500/5 rounded border border-amber-500/10">Intelligent Lead</span>
                                </div>
                            </div>
                            ...
                            <div className="space-y-3">
                                <div className="p-4 bg-zinc-900/50 rounded-xl border border-zinc-800 flex items-center gap-4">
                                     ...
                                     <div className="text-xs font-medium text-zinc-300">{selectedLead.email || 'Sem e-mail'}</div>
                                </div>
                                <div className="p-4 bg-zinc-900/50 rounded-xl border border-zinc-800 flex items-center justify-between">
                                     <div className="flex items-center gap-4">
                                          ...
                                          <div className="text-xs font-medium text-zinc-300">{selectedLead.whatsapp}</div>
                                     </div>
                                     {selectedLead.whatsapp_verified && <CheckCircle2 size={16} className="text-emerald-500" />}
                                </div>
                            </div>

                            <div className="grid grid-cols-2 gap-4">
                                <div className="bg-zinc-900/50 p-5 rounded-xl border border-zinc-800">
                                    <div className="text-[9px] font-bold text-zinc-600 uppercase tracking-widest mb-2">Estágio</div>
                                    <select 
                                        value={selectedLead.crm_stage_id || ''} 
                                        onChange={(e) => updateStage(selectedLead.id, e.target.value)}
                                        className="bg-transparent text-amber-500 font-bold text-[11px] outline-none w-full appearance-none cursor-pointer uppercase tracking-wider"
                                    >
                                        <option value="" disabled>Mudar Etapa...</option>
                                        {stages.map(s => <option key={s.id} value={s.id} className="bg-zinc-900">{s.name}</option>)}
                                    </select>
                                </div>
                                <div className="bg-zinc-900/50 p-5 rounded-xl border border-zinc-800">
                                    <div className="text-[9px] font-bold text-zinc-600 uppercase tracking-widest mb-2">Créditos</div>
                                    <div className="text-white font-bold text-sm">{selectedLead.credits || 0} <span className="text-[9px] text-zinc-600 ml-1">CR</span></div>
                                </div>
                            </div>

                            <div className="flex items-center gap-2 mb-4 border-b border-zinc-800 pb-2">
                                {['geral', 'whatsapp', 'compras', 'geracoes'].map(tab => (
                                    <button 
                                        key={tab}
                            ...
                            <div className="min-h-[200px]">
                                {drawerTab === 'geral' && (
                                    <div>
                                        <h3 className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider mb-6 flex items-center gap-2">
                                            <History size={14} className="text-amber-500" /> Notas & CRM
                                        </h3>
                                        <div className="space-y-6 relative border-l border-zinc-800 ml-2 pl-6">
                                            {interactions.map((int: any) => (
                                                <div key={int.id} className="relative">
                                                    <div className="absolute -left-[30px] top-1 w-2 h-2 rounded-full bg-zinc-800 border border-zinc-700" />
                                                    <div className="text-[9px] font-bold text-zinc-600 mb-2 uppercase tracking-wider">{new Date(int.created_at).toLocaleString()}</div>
                                                    <div className="text-xs text-zinc-400 leading-relaxed">
                                                        <span className="font-bold text-amber-500/50 text-[9px] uppercase block mb-1">{int.type}</span>
                                                        {int.content}
                                                    </div>
                                                </div>
                                            ))}
                                            {interactions.length === 0 && <div className="text-zinc-700 text-[10px] font-bold italic">Sem histórico.</div>}
                                        </div>
                                    </div>
                                )}
                                ...
                                {drawerTab === 'whatsapp' && (
                                    <div>
                                        <h3 className="text-[10px] font-bold text-emerald-400 uppercase tracking-wider mb-6 flex items-center gap-2">
                                            <MessageSquare size={14} /> Histórico WhatsApp
                                        </h3>
                                        <div className="space-y-4">
                                            {whatsappLogs.map((log: any) => (
                                                <div key={log.id} className="bg-zinc-900/50 p-4 rounded-xl border border-zinc-800">
                                                    <div className="flex items-center justify-between mb-2">
                                                        <div className="text-[9px] font-bold text-zinc-500 uppercase">{new Date(log.created_at).toLocaleString()}</div>
                                                        <div className={`text-[8px] font-bold px-2 py-0.5 rounded uppercase ${log.status === 'success' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-red-500/10 text-red-500'}`}>{log.status}</div>
                                                    </div>
                                                    <div className="text-xs text-zinc-300 whitespace-pre-wrap">{log.message}</div>
                                                    <div className="text-[9px] font-bold text-amber-500 mt-2 uppercase tracking-widest text-right">VIA: {log.instance_name || 'SYSTEM'}</div>
                                                </div>
                                            ))}
                                            {whatsappLogs.length === 0 && <div className="text-zinc-700 text-[10px] font-bold italic">Nenhuma mensagem enviada.</div>}
                                        </div>
                                    </div>
                                )}
                                ...
                                {drawerTab === 'compras' && (
                                    <div>
                                        <h3 className="text-[10px] font-bold text-blue-400 uppercase tracking-wider mb-6 flex items-center gap-2">
                                            <Zap size={14} /> Histórico de Compras
                                        </h3>
                                        <div className="space-y-4">
                                            {transactions.map((tx: any) => (
                                                <div key={tx.id} className="flex justify-between items-center bg-zinc-900/50 p-4 rounded-xl border border-zinc-800">
                                                    <div>
                                                        <div className="text-sm font-bold text-white mb-1">{tx.amount} AOA</div>
                                                        <div className="text-[9px] font-bold text-zinc-500 uppercase">{new Date(tx.created_at).toLocaleDateString()}</div>
                                                    </div>
                                                    <div className="text-right">
                                                        <div className="text-xs font-bold text-amber-500">+{tx.credits} CR</div>
                                                        <div className={`text-[9px] font-bold uppercase mt-1 ${tx.status === 'completed' ? 'text-emerald-500' : 'text-amber-500'}`}>{tx.status}</div>
                                                    </div>
                                                </div>
                                            ))}
                                            {transactions.length === 0 && <div className="text-zinc-700 text-[10px] font-bold italic">Nenhuma compra registada.</div>}
                                        </div>
                                    </div>
                                )}
                                ...
                                {drawerTab === 'geracoes' && (
                                    <div>
                                        <h3 className="text-[10px] font-bold text-purple-400 uppercase tracking-wider mb-6 flex items-center gap-2">
                                            <Layout size={14} /> Estatísticas de Uso
                                        </h3>
                                        <div className="bg-zinc-900/50 p-6 rounded-xl border border-zinc-800 text-center mb-6">
                                            <div className="text-3xl font-black text-white mb-1">{selectedLeadData?.total_generations || 0}</div>
                                            <div className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Gerações Totais</div>
                                        </div>
                                        <div className="space-y-3">
                                            {selectedLeadData?.generation_stats?.map((stat: any) => (
                                                <div key={stat.status} className="flex justify-between items-center p-3 bg-zinc-900/30 rounded-lg border border-zinc-800">
                            ...
                            <div className="pt-6 border-t border-zinc-900 space-y-3">
                                <button className="w-full flex items-center justify-center gap-2 py-3.5 bg-amber-500 text-black rounded-xl text-[11px] font-bold uppercase tracking-wider transition-all hover:opacity-90">
                                    <Bot size={16} /> Abrir Terminal Alex
                                </button>
                                <button className="w-full flex items-center justify-center gap-2 py-3.5 bg-zinc-900 border border-zinc-800 text-zinc-400 rounded-xl text-[11px] font-bold uppercase tracking-wider hover:text-white transition-all">
...
const CampaignList = () => {
    ...
    return (
         <>
                  ...
                  {filteredCampaigns.map(camp => (
                       <div key={camp.id} onClick={() => handleViewDetails(camp.id)} className="cursor-pointer group bg-zinc-900 border border-zinc-800 p-6 rounded-xl hover:border-amber-500/20 transition-all relative overflow-hidden">
                            ...
                            <div className="flex items-center justify-between pt-4 border-t border-zinc-800">
                                 ...
                                 <div className="text-[9px] font-bold text-amber-500 uppercase tracking-wider flex items-center gap-1">Relatório <ChevronRight size={10}/></div>
                            </div>
                       </div>
                  ))}
             </div>
             ...
             <AnimatePresence>
                {selectedCampaignId && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4" onClick={() => setSelectedCampaignId(null)}>
                        <motion.div onClick={e => e.stopPropagation()} initial={{ scale: 0.98, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-zinc-950 border border-zinc-800 rounded-2xl p-8 max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
                            <div className="flex justify-between items-center mb-8 shrink-0">
                                <div>
                                    <h2 className="text-xl font-bold text-white tracking-tight">Relatório de Campanha</h2>
                                    <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest mt-1">Métricas de Performance</p>
                                </div>
                                <button onClick={() => setSelectedCampaignId(null)} className="text-zinc-500 hover:text-white transition-colors"><X size={20} /></button>
                            </div>
