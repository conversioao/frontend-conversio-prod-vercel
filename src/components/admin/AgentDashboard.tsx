  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      sse.onopen = () => {
        ...
        setGlobalLogs(prev => [
          ...prev,
          { agent: 'system', message: 'Stream SSE conectado.', status: 'info', timestamp: new Date().toISOString() }
        ].slice(-100));
      };
      ...
      sse.onmessage = (e) => {
        ...
        try {
          ...
          if (log.status === 'success' || log.status === 'error') {
            ...
            setNotifications(prev => [...prev, { id, message: `${log.agent}: ${log.message}`, type: log.status === 'success' ? 'success' : 'error' }].slice(-5));
