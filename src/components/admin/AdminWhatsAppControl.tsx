...
export default function AdminWhatsAppControl() {
  ...
  const fetchMetrics = async (instance?: string) => {
    try {
      ...
      if (data.success) {
        let m = data.metrics || [];
        // If no logs exist, use agentStats as venda metric
        if (m.length === 0 && data.agentStats) {
          const total = parseInt(data.agentStats.total || 0);
          const sent = parseInt(data.agentStats.sent || 0);
          if (total > 0) {
            m = [{ category: 'venda', total, success: sent, failed: 0 }];
          }
        }
        setMetrics(m);
      }
