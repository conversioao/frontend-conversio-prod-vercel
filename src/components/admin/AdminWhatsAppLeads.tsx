...
export default function AdminWhatsAppLeads({ onClose, initialLeadId }: AdminWhatsAppLeadsProps) {
  ...
  const [ytCreateAccount, setYtCreateAccount] = useState('');
  const [ytGenerateAds, setYtGenerateAds] = useState('');
  const [ytBuyCredits, setYtBuyCredits] = useState('');
  ...
  const fetchConfig = async () => {
    try {
        const authHeader = { 'Authorization': `Bearer ${localStorage.getItem('conversio_token')}` };
        const [agentConfigRes, phoneRes, ytCreateRes, ytAdsRes, ytCreditsRes] = await Promise.all([
          fetch(`${BASE_URL}/admin/whatsapp/agent-config/${selectedInstance}`, { headers: authHeader }),
          fetch(`${BASE_URL}/admin/whatsapp/config/admin_whatsapp`, { headers: authHeader }),
          fetch(`${BASE_URL}/admin/whatsapp/config/youtube_url_create_account`, { headers: authHeader }),
          fetch(`${BASE_URL}/admin/whatsapp/config/youtube_url_generate_ads`, { headers: authHeader }),
          fetch(`${BASE_URL}/admin/whatsapp/config/youtube_url_buy_credits`, { headers: authHeader }),
        ]);

        const agentConfigData = await agentConfigRes.json();
        const phoneData = await phoneRes.json();
        const ytCreateData = await ytCreateRes.json();
        const ytAdsData = await ytAdsRes.json();
        const ytCreditsData = await ytCreditsRes.json();
        ...
        if (ytCreateData.success) setYtCreateAccount(ytCreateData.value || '');
        if (ytAdsData.success) setYtGenerateAds(ytAdsData.value || '');
        if (ytCreditsData.success) setYtBuyCredits(ytCreditsData.value || '');

        // Fetch instance-status separately with error isolation — it can be slow if Evolution API is down
        fetch(`${BASE_URL}/admin/whatsapp/instance-status`, { headers: authHeader })
          .then(r => r.json())
          .then(statusData => { if (statusData.success) setInstanceStatus(statusData); })
          .catch(e => console.warn('[fetchConfig] instance-status failed (non-blocking):', e.message));
  ...
  const syncChats = async (silentMode = false) => {
    setSyncing(true);
    try {
      ...
      if (data.success) {
        await fetchLeads();
        if (!silentMode) alert(`Sincronização concluída! ${data.syncedCount} conversas processadas.`);
      } else {
        if (!silentMode) alert(`Erro na sincronização: ${data.message}`);
        else console.warn('[Sync]', data.message);
      }
    } catch (e) {
      console.error('[Sync Error]', e);
      if (!silentMode) alert('Erro ao sincronizar com Evolution API.');
    } finally {
      setSyncing(false);
    }
  };
  ...
  const saveAgentConfig = async () => {
    ...
    try {
      ...
      // Save YouTube URLs in parallel
      await Promise.all([
        fetch(`${BASE_URL}/admin/whatsapp/config`, {
          method: 'POST', headers: authHeader,
          body: JSON.stringify({ key: 'youtube_url_create_account', value: ytCreateAccount })
        }),
        fetch(`${BASE_URL}/admin/whatsapp/config`, {
          method: 'POST', headers: authHeader,
          body: JSON.stringify({ key: 'youtube_url_generate_ads', value: ytGenerateAds })
        }),
        fetch(`${BASE_URL}/admin/whatsapp/config`, {
          method: 'POST', headers: authHeader,
          body: JSON.stringify({ key: 'youtube_url_buy_credits', value: ytBuyCredits })
        }),
      ]);

      if (data.success) {
        alert('Configurações do agente e links do YouTube guardados com sucesso!');
        fetchConfig();
      } else {
  ...
  useEffect(() => {
    ...
    setMessages([]);
    // Auto-sync on mount and instance switch (silent)
    syncChats(true);
        ...
        <div className="flex items-center gap-8">
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-2 bg-zinc-900/50 p-1 rounded-xl border border-zinc-800">
              {/* Always show tabs — use instanceStatus for state indicators if available, fallback to static tabs */}
              {(['venda', 'conversio-sistema'] as const).map((instId) => {
                const instInfo = instanceStatus?.instances?.find((i: any) => i.instanceName === instId);
                const label = instId === 'conversio-sistema' ? 'SISTEMA' : 'VENDA';
                const state = instInfo?.state;
                return (
                  <button 
                    key={instId} 
                    onClick={() => setSelectedInstance(instId)}
                  ...
                  >
                    <div className={`w-1.5 h-1.5 rounded-full ${
                      state === 'open' ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]' 
                      : state ? 'bg-red-500' 
                               ...
                               <div className="flex items-center justify-between gap-8 mb-2 opacity-40 text-[9px] uppercase font-bold tracking-wider">
                                  <span>
                                    {msg.role === 'user' ? 'Cliente' : 
                                     ...
                                     msg.role === 'human' ? 'Operador Humano' : `${agentName || 'Alex'} (IA)`}
            ...
            <div className="max-w-4xl mx-auto w-full space-y-12">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                 ...
                 <div className="space-y-6">
                    ...
                    <div className="p-6 bg-zinc-900/50 border border-zinc-800 rounded-xl space-y-5">
                       <div className="flex items-center gap-2">
                          <span className="text-lg">▶️</span>
                          <h6 className="text-xs font-bold text-white uppercase tracking-wider">Tutoriais YouTube</h6>
                       </div>
                       <p className="text-[10px] text-zinc-500 leading-relaxed">O agente envia estes links automaticamente quando o lead pergunta como usar a plataforma.</p>
                       
                       <div className="space-y-2">
                          <label className="text-[9px] text-zinc-500 font-bold uppercase tracking-wider block">🔵 Como Criar Conta</label>
                          <input
                            type="url"
                            value={ytCreateAccount}
                            onChange={(e) => setYtCreateAccount(e.target.value)}
                            placeholder="https://www.youtube.com/watch?v=..."
                            className="w-full bg-zinc-950 border border-zinc-700 rounded-lg p-2.5 text-[11px] text-zinc-300 focus:outline-none focus:border-[#FFB800]/50 transition-all"
                          />
                       </div>
                       
                       <div className="space-y-2">
                          <label className="text-[9px] text-zinc-500 font-bold uppercase tracking-wider block">🟡 Como Gerar Anúncios</label>
                          <input
                            type="url"
                            value={ytGenerateAds}
                            onChange={(e) => setYtGenerateAds(e.target.value)}
                            placeholder="https://www.youtube.com/watch?v=..."
                            className="w-full bg-zinc-950 border border-zinc-700 rounded-lg p-2.5 text-[11px] text-zinc-300 focus:outline-none focus:border-[#FFB800]/50 transition-all"
                          />
                       </div>
                       ...
                       <div className="space-y-2">
                          <label className="text-[9px] text-zinc-500 font-bold uppercase tracking-wider block">🟢 Como Comprar Créditos</label>
                          <input
                            type="url"
                            value={ytBuyCredits}
                            onChange={(e) => setYtBuyCredits(e.target.value)}
                            placeholder="https://www.youtube.com/watch?v=..."
                            className="w-full bg-zinc-950 border border-zinc-700 rounded-lg p-2.5 text-[11px] text-zinc-300 focus:outline-none focus:border-[#FFB800]/50 transition-all"
                          />
                       </div>
