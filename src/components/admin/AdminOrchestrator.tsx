...
export default function AdminOrchestrator({ onClose }: { onClose: () => void }) {
    ...
    const [activeTab, setActiveTab] = useState<'plans' | 'terminal' | 'chat' | 'approvals' | 'leads'>('plans');
    ...
    const totalApprovalsCount = pendingTasks.length + pendingCampaigns.length;
    ...
    useEffect(() => {
        ...
        // Polling para novos planos a cada 2 min
    ...
    const fetchAutopilotState = async () => {
        try {
            const res = await apiFetch('/admin/configs/orchestrator_autopilot');
            const data = await res.json();
            if (data.success) setIsAutopilot(data.value === 'true');
        } catch (e) {}
    };

    const fetchRecentLeads = async () => {
        try {
            const res = await apiFetch('/agent/contacts?admin_leads=true');
            const data = await res.json();
            if (data.success) setRecentLeads(data.contacts || []);
        } catch (e) {}
    };
    ...
    const toggleAutopilot = async () => {
        ...
        try {
            await apiFetch('/admin/configs', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ key: 'orchestrator_autopilot', value: newValue ? 'true' : 'false' })
            });
        } catch (e) {
            setIsAutopilot(!newValue);
        }
                ...
                <div className="flex items-center gap-6 pointer-events-auto">
                    ...
                    <div className="flex items-center gap-3 bg-white/5 border border-white/10 px-4 py-2 rounded-xl">
                        <div className="flex flex-col">
                            <span className="text-[10px] text-zinc-400 uppercase tracking-wider font-bold">Piloto Automático</span>
                            <span className={`text-[11px] font-medium ${isAutopilot ? 'text-emerald-400' : 'text-zinc-400'}`}>
                                {isAutopilot ? 'MODO AUTÓNOMO' : 'VALIDAÇÃO MANUAL'}
                            </span>
                        </div>
                        <button
                            onClick={toggleAutopilot}
                            className={`relative w-12 h-6 rounded-full transition-all duration-300 ${isAutopilot ? 'bg-emerald-500' : 'bg-zinc-700'}`}
                        >
                            <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all duration-300 ${isAutopilot ? 'left-7' : 'left-1'}`} />
                        </button>
                    </div>

                    <button onClick={onClose} className="p-3 bg-red-500/20 hover:bg-red-500/40 text-red-500 rounded-full transition-all backdrop-blur-md">
                        <X size={24} />
                    </button>
                </div>
            </div>
                ...
                <div className="flex-1 h-full flex flex-col">
                    ...
                    <div className="flex items-center gap-1 px-6 pt-4 border-b border-zinc-800 shrink-0">
                        ...
                        <button
                            ...
                            className={`px-4 py-2.5 rounded-t-lg text-[10px] font-bold uppercase tracking-widest transition-all flex items-center gap-2 ${
                                activeTab === 'leads' ? 'bg-zinc-900 text-emerald-500 border-b-2 border-emerald-500' : 'text-zinc-600 hover:text-zinc-400'
                            }`}
                        >
                            <Users size={14} />
                            Leads
                            {recentLeads.filter(l => l.needs_human).length > 0 && (
                                <span className="px-1.5 py-0.5 rounded-full bg-amber-500 text-black text-[8px] font-black">
                                    {recentLeads.filter(l => l.needs_human).length}
                                </span>
                            )}
                        </button>
                    </div>
                                ...
                                >
                                    ...
                                    {recentLeads.length === 0 ? (
                                        <div className="flex-1 flex flex-col items-center justify-center text-zinc-600">
                                            ...
                                            <p>Nenhum lead detectado recentemente.</p>
                                        </div>
                                    ) : (
                                        <div className="grid gap-4">
