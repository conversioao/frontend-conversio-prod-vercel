...
export function AdminDashboard() {
  ...
  const fetchStats = async () => {
    try {
      ...
      const execRes = await apiFetch('/admin/dashboard/executive');
      const execData = await execRes.json();
      if (execData.status === 'success') {
        setExecutiveStats(execData.data);
      }
    } catch (err) {
      console.error(err);
    } finally {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 bg-surface/50 backdrop-blur-md border border-border-subtle p-2 rounded-2xl">
          <div className="flex items-center gap-2 px-4 py-2 bg-emerald-500/10 text-emerald-500 rounded-xl border border-emerald-500/20">
            ...
            <span className="text-xs font-bold uppercase tracking-wider">{stats.activeUsersCount || 0} Usuários Live</span>
          </div>
          <div className="flex items-center gap-2 px-4 py-2 bg-[#FFB800]/10 text-[#FFB800] rounded-xl border border-[#FFB800]/20">
             <Zap size={14} fill="currentColor" />
             <span className="text-xs font-bold uppercase tracking-wider">{stats.activeProcessing || 0} Processando Agora</span>
          </div>
          <button 
      ...
      {/* C-Level Orchestrator Metrics Section */}
      {executiveStats && (
        <div className="bg-surface border border-border-subtle p-8 rounded-[2.5rem] shadow-sm mb-6 mt-8">
           <div className="flex items-center justify-between mb-8">
              <div className="flex items-center gap-3">
                 <div className="w-10 h-10 rounded-xl bg-purple-500/10 flex items-center justify-center text-purple-500">
                    <Activity size={20} />
                 ...
                 <div>
                    <h3 className="text-lg font-bold text-text-primary">Orquestrador C-Level (Operação Autônoma)</h3>
                    <p className="text-xs text-text-tertiary">Métricas vitais de funil e saúde sistêmica</p>
                 </div>
              </div>
           </div>
           
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <div className="bg-bg-base border border-border-subtle p-6 rounded-3xl flex flex-col">
                 <p className="text-[10px] font-black text-emerald-500 uppercase tracking-widest mb-2">Leads Convertidos</p>
                 <h4 className="text-3xl font-black text-text-primary">
                   {executiveStats.funnel?.find((f: any) => f.status === 'converted')?.total || 0}
                 </h4>
              </div>

              <div className="bg-bg-base border border-border-subtle p-6 rounded-3xl flex flex-col">
                 <p className="text-[10px] font-black text-[#FFB800] uppercase tracking-widest mb-2">Fila de Tarefas Pendentes</p>
                 <h4 className="text-3xl font-black text-text-primary">
                   {executiveStats.orchestrator_queue?.find((q: any) => q.status === 'pending')?.total || 0}
                 </h4>
              </div>

              <div className="bg-bg-base border border-border-subtle p-6 rounded-3xl flex flex-col">
                 <p className="text-[10px] font-black text-red-500 uppercase tracking-widest mb-2">Erros / Falhas (Auto-Healing)</p>
                 <h4 className="text-3xl font-black text-text-primary">
                   {executiveStats.orchestrator_queue?.find((q: any) => q.status === 'failed')?.total || 0}
                 </h4>
              </div>

              <div className="bg-bg-base border border-border-subtle p-6 rounded-3xl flex flex-col">
                 <p className="text-[10px] font-black text-blue-500 uppercase tracking-widest mb-2">Presunção de Receita (Pendente)</p>
                 <h4 className="text-3xl font-black text-text-primary">
                   {Number(executiveStats.revenue_metrics?.find((r: any) => r.status === 'pending')?.total_value || 0).toLocaleString()} <span className="text-sm">Kz</span>
                 </h4>
              </div>
           </div>

           <div className="mt-8 pt-6 border-t border-border-subtle">
             <h4 className="text-sm font-bold text-text-secondary mb-4 uppercase tracking-widest">Top 5 Campanhas Recentes</h4>
             <div className="flex flex-wrap gap-4">
                {executiveStats.top_campaigns?.length > 0 ? executiveStats.top_campaigns.map((camp: any, idx: number) => (
                  <div key={idx} className="px-4 py-2 bg-bg-base border border-border-subtle rounded-xl flex items-center justify-between min-w-[200px]">
                    <span className="text-xs font-bold text-text-secondary">{camp.type}</span>
                    <span className="text-xs font-black text-[#FFB800] bg-[#FFB800]/10 px-2 py-1 rounded-md">{camp.interactions} hit</span>
                  </div>
                )) : (
                  <p className="text-xs text-text-tertiary">Nenhuma interação registrada recentemente.</p>
                )}
             </div>
           </div>
        </div>
