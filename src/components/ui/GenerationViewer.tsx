...
import { X, ChevronLeft, ChevronRight, Mic, Music, Check, Globe, Download, Loader2, CheckCircle2, AlertCircle, Edit2, Copy } from 'lucide-react';
...
export function GenerationViewer({ 
  ...
  const isAudio = item.type === 'musica' || item.type === 'audio' || item.type === 'music';
  const isVoice = item.type === 'voice' || isAudio;
  const isMusic = isAudio || !!item.metadata?.lyric;

  const safeCopy = getSafeText(item.metadata?.copy || item.metadata?.copy_anuncio || item.copy || item.metadata?.lyric);
    ...
    <>
      ...
      >
        ...
        <div className="flex-1 flex items-center justify-center relative overflow-hidden bg-black">
          {item.status === 'processing' || !item.result_url ? (
            <div className="flex flex-col items-center gap-4 text-center px-6">
              <div className="relative">
                <div className="w-16 h-16 border-4 border-[#FFB800]/20 border-t-[#FFB800] rounded-full animate-spin" />
                <Loader2 className="absolute inset-0 m-auto text-[#FFB800] animate-pulse" size={24} />
              </div>
              <div className="animate-in fade-in slide-in-from-bottom-2 duration-700">
                <h3 className="text-xl font-bold text-white mb-2">Finalizando sua obra...</h3>
                <p className="text-sm text-text-tertiary">O mestre IA está dando os toques finais. Quase lá!</p>
              </div>
            </div>
          ) : item.status === 'failed' ? (
            <div className="flex flex-col items-center gap-4 text-center px-6">
              ...
              <h3 className="text-xl font-bold text-white">Ops! Algo correu mal</h3>
              <p className="text-sm text-text-tertiary max-w-[240px]">{item.metadata?.error || 'Não foi possível gerar este conteúdo. Tente novamente.'}</p>
            </div>
          ) : isVideo ? (
            <video
              src={item.result_url}
            ...
            <div className="flex flex-col items-center gap-6 p-8 w-full">
              <div className="w-24 h-24 rounded-full bg-[#FFB800]/20 flex items-center justify-center animate-pulse">
                {isMusic ? <Music size={48} className="text-[#FFB800]" /> : <Mic size={48} className="text-[#FFB800]" />}
              </div>
              {item.metadata?.title && <h2 className="text-lg font-bold text-white text-center line-clamp-2">{item.metadata.title}</h2>}
          ...
          <div className="relative flex-1 bg-black flex items-center justify-center min-h-[40vh] lg:min-h-0 overflow-hidden">
            {item.status === 'processing' || !item.result_url ? (
              <div className="flex flex-col items-center gap-6 text-center max-w-sm px-8">
                <div className="relative">
                  <div className="w-24 h-24 border-8 border-[#FFB800]/10 border-t-[#FFB800] rounded-full animate-spin" />
                  <div className="absolute inset-0 m-auto w-12 h-12 bg-[#FFB800]/20 rounded-full flex items-center justify-center animate-pulse">
                    <Loader2 className="text-[#FFB800]" size={24} />
                  </div>
                </div>
                <div>
                  <h3 className="text-2xl font-black text-white mb-3 tracking-tight uppercase">Processando...</h3>
                  <p className="text-sm text-text-tertiary leading-relaxed">
                    Estamos a criar algo extraordinário. <br/>
                    Este processo pode demorar até 1 minuto.
                  </p>
                </div>
              </div>
            ) : item.status === 'failed' ? (
              <div className="flex flex-col items-center gap-6 text-center max-w-sm px-8">
                <div className="w-24 h-24 bg-red-500/10 rounded-full flex items-center justify-center text-red-500 shadow-[0_0_40px_rgba(239,68,68,0.2)] border border-red-500/20">
                  <AlertCircle size={48} />
                </div>
                <div>
                  <h3 className="text-2xl font-black text-white mb-3 tracking-tight uppercase">Geração Falhou</h3>
                  <p className="text-sm text-red-400/80 leading-relaxed font-medium">
                    {item.metadata?.error || 'Não foi possível concluir a geração deste conteúdo.'}
                  </p>
                </div>
              </div>
            ) : isVideo ? (
              ...
              <div className="w-full max-w-lg aspect-square flex flex-col items-center justify-center p-12 bg-white/5 rounded-[3rem] border border-white/10 relative overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-br from-[#FFB800]/5 to-transparent pointer-events-none" />
                <div className="w-32 h-32 rounded-full bg-[#FFB800]/20 flex items-center justify-center text-[#FFB800] mb-6 animate-pulse relative z-10 shadow-[0_0_50px_rgba(255,184,0,0.2)]">
                  {isMusic ? <Music size={56} /> : <Mic size={56} />}
                </div>
                {item.metadata?.title && (
                  <h2 className="text-xl font-black text-white text-center mb-2 tracking-tight z-10 px-4 line-clamp-2">
                    {item.metadata.title}
                  </h2>
                )}
                <p className="text-[10px] font-black text-white/40 uppercase tracking-[0.2em] mb-8 z-10">
                  {isMusic ? 'Música Original IA' : 'Voz Gerada'}
                </p>
                <audio src={item.result_url} className="w-full h-12 relative z-10" controls autoPlay />
              </div>
            ) : (
...
function RenderCopy({ copy }: { copy: any }) {
  ...
  // Handle Music Lyrics first
  const lyric = copy.metadata?.lyric || copy.lyric;
  if (copy.type === 'music' || lyric) {
    if (lyric) {
      return (
        <div className="flex flex-col gap-4">
          <div className="p-4 bg-white/5 rounded-xl border border-white/10">
            <span className="text-[9px] font-black text-[#FFB800] uppercase block mb-2 tracking-[0.2em]">Letra da Música</span>
            <p className="text-white/80 text-sm leading-relaxed italic whitespace-pre-wrap">
              {lyric}
            </p>
          </div>
        </div>
    ...
    if (copy.type === 'music') return <span className="text-white/40 italic">Música instrumental ou sem letra disponível.</span>;
  }

  // Handle standard copy text
  const textContent = copy.metadata?.copy || copy.metadata?.copy_anuncio || copy.copy || copy.metadata?.caption || copy.metadata?.text || copy.metadata?.prompt;
  if (!textContent) return <span>Crie uma legenda para esta imagem.</span>;

  let parsedCopy = textContent;
  if (typeof textContent === 'string' && (textContent.startsWith('{') || textContent.startsWith('['))) {
    try { parsedCopy = JSON.parse(textContent); } catch(e) {}
  }
