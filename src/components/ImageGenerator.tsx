...
export function ImageGenerator({ initialCore, onClearCore, onProgressUpdate }: ImageGeneratorProps = {}) {
  ...
  // isKit = any agent with category === 'branding' (covers all 6 branding kits)
  const isKit = selectedCore?.category === 'branding';
  ...
  // For kits: count of checked styles (default = all styles); for regular agents: use quantity picker
  const totalQuantity = (isKit && activeStyleConfigs.length > 0)
    ? activeStyleConfigs.length
  ...
  // Kit cost: base kit cost + (model + surcharges) * number of active styles
  const totalCost = isKit
              ...
              {wizardStep === 2 && selectedCore && (
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500 mb-10 pb-4">
                        ...
                        <div className="p-4 rounded-2xl bg-[#FFB800]/5 border border-[#FFB800]/10 flex items-start gap-3">
                          ...
                          <div className="space-y-1">
                            ...
                            <p className="text-[8px] text-text-tertiary leading-relaxed opacity-70">
                              {totalQuantity} frame{totalQuantity !== 1 ? 's' : ''} selecionado{totalQuantity !== 1 ? 's' : ''} — selecione os estilos que pretende gerar na coluna da direita.
                            </p>
                          </div>
                        </div>
                      )}
                  ...
                  <div className="lg:col-span-7 space-y-6">
                    ...
                    {/* IF BRANDING KIT: Show interactive checklist frames (all checked by default, user can deselect) */}
                    {isKit && (
                      <div className="space-y-4">
                        <div className="flex items-center justify-between px-2">
                          <div className="flex items-center gap-3">
                            <div className="w-1.5 h-4 bg-[#FFB800] rounded-full" />
                            <h4 className="text-[11px] font-black text-white uppercase tracking-[0.2em]">Frames de Campanha</h4>
                          </div>
                          <span className="text-[9px] text-[#FFB800]/60 font-bold">{totalQuantity} selecionado{totalQuantity !== 1 ? 's' : ''}</span>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[500px] overflow-y-auto pr-1 scrollbar-thin">
                          {(AGENT_STYLES[selectedCore.agentTechnicalId] || AGENT_STYLES[selectedCore.id] || []).map((s: any, idx: number) => {
                            const config = styleConfigs[s.name] || { checked: true, quantity: 1, aspectRatio: '9:16' };
                            const isSelected = config.checked;
                            const isLastActive = isSelected && activeStyleConfigs.length === 1;
                            const Icon = s.icon || Sparkles;
                            return (
                              <div
                                key={s.id}
                                onClick={() => {
                                  if (isLastActive) return; // prevent deselecting last style
                                  setStyleConfigs(prev => ({ ...prev, [s.name]: { ...config, checked: !config.checked } }));
                                }}
                                className={`flex items-start gap-3 p-4 rounded-3xl border transition-all group ${
                                  isLastActive ? 'cursor-not-allowed opacity-80' : 'cursor-pointer'
                                } ${isSelected
                                  ? 'bg-[#FFB800]/5 border-[#FFB800]/40 shadow-[0_0_20px_rgba(255,184,0,0.06)]'
                                  : 'bg-white/[0.02] border-white/5 hover:border-white/10'
                                }`}
                              >
                                <div className={`w-9 h-9 shrink-0 rounded-2xl flex items-center justify-center transition-all ${
                                  isSelected ? 'bg-[#FFB800]/10 text-[#FFB800]' : 'bg-white/5 text-text-tertiary group-hover:text-white'
                                }`}>
                                ...
                                <div className="flex-1 space-y-1 min-w-0">
                                  ...
                                  <p className="text-[8.5px] text-text-tertiary leading-relaxed opacity-70 line-clamp-2">{s.desc}</p>
                                </div>
                                <div className={`w-5 h-5 shrink-0 rounded-full border-2 flex items-center justify-center transition-all ${
                                  isSelected ? 'bg-[#FFB800] border-[#FFB800]' : 'border-white/10 group-hover:border-white/20'
                                }`}>
