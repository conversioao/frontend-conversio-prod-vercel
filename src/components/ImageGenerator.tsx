...
import { ImagePlus, Sparkles, Smartphone, Copy, Mic, Send, Download, ThumbsUp, MoreHorizontal, X, Plus, Maximize2, ArrowRight, Globe, Loader2, CheckCircle2, AlertCircle, Zap, Trash2, Layers, ChevronRight, Camera, Package, MessagesSquare, MapPin, TrendingUp, MonitorPlay, BookOpen, Target, Users, Search, Box } from 'lucide-react';
...
const AGENT_CORES = [
  { id: 'ugc-lite', name: 'REELANGOLA LITE', agentTechnicalId: 'ugc-lite', style_id: 'ugc-lite', icon: Zap, description: 'Versão rápida e económica para testes e criativos simples.', credit_cost: 2 },
  { id: 'ugc-realistic', name: 'REELANGOLA UGC', agentTechnicalId: 'ugc-realistic', style_id: 'ugc-realistic', icon: Smartphone, description: 'Conteúdo autêntico e orgânico (Talking Head, Unboxing, Routine).', credit_cost: 4 },
  { id: 'boutique-fashion', name: 'LUANDALOOKS AGENT', agentTechnicalId: 'boutique-fashion', style_id: 'boutique-fashion', icon: Sparkles, description: 'Especialista em Publicidade de Moda para Angola.', credit_cost: 4 },
  { id: 'glow-angola', name: 'GLOWANGOLA PRO', agentTechnicalId: 'glow-angola', style_id: 'glow-angola', icon: Sparkles, description: 'Especialista em Cosmética, Cabelo e Beleza para Angola.', credit_cost: 4 },
  { id: 'impact-ads-pro', name: 'VIBRA ANGOLA', agentTechnicalId: 'impact-ads-pro', style_id: 'impact-ads-pro', icon: Zap, description: 'Anúncios de alto impacto com inteligência visual.', credit_cost: 5 },
];
...
export function ImageGenerator({ initialCore, onClearCore, onProgressUpdate }: ImageGeneratorProps = {}) {
  ...
  const totalQuantity = isAgentWithStyles && activeStyleConfigs.length > 0
    ? activeStyleConfigs.reduce((sum, c) => sum + c.quantity, 0)
    : quantity;
    
  const surcharge = (generationMode === 'standard' && (capturedFile || uploadedImage) && selectedCore) ? compositionSurcharge : 0;
  // Wait, I will just use unityCost for totalCost here to display at the bottom
  const totalCost = (unitCost + surcharge) * totalQuantity;
  ...
  const fetchModels = async () => {
    try {
      ...
      if (modelsData.success) {
        ...
        // Default to nano-banana-lite if available
        const defaultModel = models.find((m: any) => m.style_id === 'nano-banana-lite' || m.id === 'nano-banana-lite');
      ...
      if (coresData.success && coresData.models && coresData.models.length > 0) {
        ...
        // Default to UGC if available, otherwise active core
        const ugcCore = mergedCores.find((c: any) => c.agentTechnicalId === 'ugc-realistic' || c.id === 'ugc-realistic');
        setSelectedCore(ugcCore || mergedCores.find((c: any) => c.is_active) || mergedCores[0]);
      }
    } catch (e) {}
  };
  ...
  const handleGenerate = async () => {
    ...
    // Track generation event for funnel scoring
    ...
    // UX: Immediate feedback and return to idle to allow a new generation
    ...
    // Remove the 3s reset - let SSE or completion handle state transition
    // setTimeout(() => {
    //   setStatus('idle');
    // }, 3000);
    ...
    try {
      ...
      if (data.success) {
        ...
        // UX: Show "Estamos a processar" after 5 seconds
        setTimeout(() => {
          setConfirmModalConfig({
        ...
        if (data.batchId) {
          ...
          // Return to idle state to allow next generation
          setStatus('idle');
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-9rem)] w-full max-w-6xl mx-auto relative px-4 sm:px-0 overflow-visible">
      ...
      {(status === 'done' || (status === 'generating' && sseProgress)) && (
        <div className="w-full max-w-4xl mx-auto mt-4 mb-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className={`grid gap-6 ${
            ...
            {(generatedItems.length > 0 ? generatedItems : Array.from({ length: quantity })).map((item: any, idx) => (
              ...
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2 flex-wrap overflow-visible">
                   ...
                   <div className="relative overflow-visible">
                    ...
                    {/* Modo Composição Toggle removed */}
        ...
        >
          ...
          >
            ...
            <div className="overflow-y-auto px-10 pb-10 flex-1 custom-scrollbar">
                  ...
                  {availableCores.map((core: any) => {
                    ...
                    return (
                      <button
                        ...
                        onClick={() => { 
                          setSelectedCore(core); 
                          // Initialize multi-style configs
                          ...
                          coreStyles.forEach((s: any, idx: number) => {
                              initialConfigs[s.name] = { checked: idx === 0, quantity: 1, aspectRatio: '9:16' };
                          });
                          setStyleConfigs(initialConfigs);
                          setWizardStep(2); // One-click advance
                        }}
              ...
              {wizardStep === 2 && selectedCore && (AGENT_STYLES[selectedCore.agentTechnicalId] || AGENT_STYLES[selectedCore.id]) && (
                <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500 mb-8">
                  <h4 className="text-xs font-semibold text-text-tertiary uppercase tracking-widest px-1">
                    Selecione os Estilos (Múltipla Escolha)
                  </h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {(AGENT_STYLES[selectedCore.agentTechnicalId as keyof typeof AGENT_STYLES] || AGENT_STYLES[selectedCore.id as keyof typeof AGENT_STYLES] || []).map((s: any) => {
                      ...
                      return (
                        <div key={s.id} className={`flex flex-col p-4 rounded-2xl border transition-all ${
                            isSelected 
                              ? 'bg-[#FFB800]/5 border-[#FFB800]/40 shadow-[0_0_20px_rgba(255,184,0,0.15)]' 
                              : 'bg-white/[0.02] border-white/5 hover:border-white/10'
                          }`}>
                            <div className="flex items-center gap-3 mb-4 cursor-pointer" onClick={() => {
                                setStyleConfigs(prev => ({
                                    ...prev,
                                    [s.name]: { ...config, checked: !config.checked }
                                }));
                            }}>
                              <div className={`w-8 h-8 rounded-xl flex items-center justify-center transition-colors ${isSelected ? 'bg-[#FFB800] text-black' : 'bg-white/5 text-text-tertiary group-hover:text-white'}`}>
                                <Icon size={16} />
                              </div>
                              <div className="flex-1">
                                <span className={`text-sm font-bold block leading-tight ${isSelected ? 'text-white' : 'text-text-secondary'}`}>{s.name}</span>
                                {s.desc && <span className="text-[10px] font-normal opacity-60 leading-tight block text-text-tertiary mt-1">{s.desc}</span>}
                              </div>
                              <div className={`w-5 h-5 shrink-0 rounded-full border flex items-center justify-center ${isSelected ? 'bg-[#FFB800] border-[#FFB800]' : 'border-white/20'}`}>
                                {isSelected && <CheckCircle2 size={12} className="text-black" />}
                              </div>
                            </div>
                            
                            {/* Settings for this style */}
                            {isSelected && (
                                <div className="mt-2 pt-3 border-t border-white/5 flex flex-col gap-3 animate-in fade-in slide-in-from-top-2 duration-300">
                                    <div className="flex justify-between items-center text-xs">
                                        <span className="text-text-tertiary">Formato</span>
                                        <select 
                                            value={config.aspectRatio} 
                                            onChange={e => setStyleConfigs(prev => ({ ...prev, [s.name]: { ...config, aspectRatio: e.target.value } }))}
                                            className="bg-black/50 border border-white/10 rounded px-2 py-1.5 outline-none text-[#FFB800] cursor-pointer"
                                        >
                                            {RATIOS.map(r => <option key={r.id} value={r.id}>{r.id}</option>)}
                                        </select>
                                    </div>
                                    <div className="flex justify-between items-center text-xs">
                                        <span className="text-text-tertiary">Quantidade (Máx 10)</span>
                                        <input 
                                            type="number" 
                                            min="1" max="10" 
                                            ...
                                            onChange={e => {
                                                ...
                                                if (val > 10) val = 10;
                                                if (val < 1) val = 1;
                                                setStyleConfigs(prev => ({ ...prev, [s.name]: { ...config, quantity: val } }));
                                            }}
                                            className="bg-black/50 border border-white/10 rounded w-16 px-2 py-1.5 outline-none text-[#FFB800] text-center"
                                        />
                                    </div>
                                </div>
                            )}
              ...
              {wizardStep === 2 && (
                <div className="flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  ...
                  {(selectedCore?.agentTechnicalId === 'impact-ads-pro' || 
                    selectedCore?.agentTechnicalId === 'boutique-fashion' || 
                    selectedCore?.agentTechnicalId === 'glow-angola' || 
                    selectedCore?.agentTechnicalId === 'ugc-realistic') && (
