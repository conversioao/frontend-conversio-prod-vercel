    ...
    try {
      ...
      if (data.success) {
        ...
        data.generations.forEach((gen: any) => {
          ...
          const batchTitle = gen.title || gen.metadata?.title;
          const batchCopy = gen.copy || gen.metadata?.lyrics || gen.metadata?.copy;
          ...
          if (batchTitle && !batch.title) batch.title = batchTitle;
        });
