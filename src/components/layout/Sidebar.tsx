...
import { Home, Image as ImageIcon, Video, Mic, Compass, Folder, CreditCard, Settings, AlertCircle, X, LogOut, ShieldAlert, ShieldCheck, LayoutDashboard, Headphones, Bot, PenTool, Film, Sparkles, Music, ChevronRight, Target } from 'lucide-react';
...
export function Sidebar({ activePage = 'home', onNavigate, onLogout, isOpen = false, onClose, user, isGeneratingImage, isGeneratingVideo, isGeneratingAudio, unreadCount = 0 }: SidebarProps) {
  ...
  const navItems = [
    ...
    { id: 'agent-whatsapp-setup', icon: Bot, label: 'Agente WhatsApp', visible: false, badge: unreadCount },
    { id: 'agent-whatsapp-crm', icon: Target, label: 'CRM Vendas', visible: false, badge: unreadCount },
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="relative">
                          ...
                          {!isActive && (item as any).badge > 0 && (
                            <div className="absolute -top-1 -right-1 min-w-[14px] h-[14px] px-1 bg-red-500 rounded-full flex items-center justify-center text-[8px] font-black text-white border border-[#0A0A0A]">
                                {(item as any).badge}
                            </div>
                          )}
                        </div>
                ...
                return (
                  ...
                  >
                    <div className="relative">
                      ...
                      {!isActive && (item as any).badge > 0 && (
                        <div className="absolute -top-1 -right-1 min-w-[16px] h-[16px] px-1 bg-red-500 rounded-full flex items-center justify-center text-[8px] font-black text-white border-2 border-surface shadow-lg">
                            {(item as any).badge}
                        </div>
                      )}
                    </div>
